        ...
        <motion.div key={active} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }}
          ...
          {active === "profile" && (
            <FadeUp>
              ...
              <div className="flex items-center gap-5 mb-8 pb-8" style={{ borderBottom: `1px solid ${border}` }}>
                <div className="rounded-full flex items-center justify-center text-2xl font-bold" style={{ width: 72, height: 72, background: "#6357E8", color: "white", fontFamily: "'Bricolage Grotesque', sans-serif" }}>A</div>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 600, color: fg, marginBottom: 4 }}>Ada Lovelace</div>
                  <div style={{ fontSize: 13, color: sub, marginBottom: 10 }}>ada@example.com</div>
              ...
              {[
                ...
                { label: "Email address", value: email, onChange: () => {}, type: "email" },
                { label: "Handle", value: "@ada", onChange: () => {}, type: "text" },
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ fontSize: 13, fontWeight: 500, color: fg, display: "block", marginBottom: 6 }}>Bio</label>
                <textarea rows={3}
                  defaultValue="Building the future of note-taking. Interested in distributed systems and cognitive science."
                  style={{ width: "100%", background: inputBg, border: `1px solid ${border}`, borderRadius: 10, padding: "11px 14px", fontSize: 14, color: fg, outline: "none", fontFamily: "'DM Sans', sans-serif", boxSizing: "border-box", resize: "none" }}
                  onFocus={e => (e.target.style.borderColor = "#6357E8")}
                  onBlur={e => (e.target.style.borderColor = border)} />
              </div>
          ...
          {active === "appearance" && (
            <FadeUp>
              ...
              <div style={{ marginBottom: 32 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: fg, marginBottom: 4 }}>Font size</div>
                <p style={{ fontSize: 13, color: sub, marginBottom: 14 }}>Adjusts the editor and sidebar text size.</p>
                <div className="flex gap-2">
                  {["Small", "Default", "Large", "X-Large"].map((size, i) => (
                    <button key={size} onClick={() => {}}
                      className="rounded-lg border transition-all"
                      style={{ padding: "7px 14px", fontSize: 13 - i * 0.5 + "px", background: i === 1 ? "rgba(99,87,232,0.1)" : card, borderColor: i === 1 ? "#6357E8" : border, color: i === 1 ? "#6357E8" : sub, cursor: "pointer", borderWidth: i === 1 ? 2 : 1 }}>
                      {size}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: fg, marginBottom: 4 }}>Accent color</div>
                <p style={{ fontSize: 13, color: sub, marginBottom: 14 }}>Used for links, buttons, and highlights.</p>
                <div className="flex gap-3">
                  {["#6357E8", "#22C27D", "#F59E0B", "#EC4899", "#3B82F6", "#0E0E0C"].map(color => (
                    <button key={color} onClick={() => {}}
                      className="rounded-full transition-transform hover:scale-110"
                      style={{ width: 28, height: 28, background: color, border: color === "#6357E8" ? "3px solid rgba(99,87,232,0.4)" : "2px solid transparent", cursor: "pointer", outline: "none" }} />
                  ))}
                </div>
              </div>
            </FadeUp>
          ...
          {active === "privacy" && (
            <FadeUp>
              ...
              {[
                ...
                { title: "Active sessions", desc: "You are signed in on 2 devices. Last active: MacBook Pro (now).", btn: "Manage sessions", btnColor: fg },
            ...
            <FadeUp>
              ...
              <div className="rounded-2xl border mb-8" style={{ padding: 28, background: card, borderColor: border, position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ marginTop: 20, paddingTop: 20, borderTop: `1px solid ${border}` }}>
                  <div style={{ fontSize: 13, color: sub }}>Everything is included. No upgrades needed.</div>
                </div>
              </div>

              <div style={{ fontSize: 13, fontWeight: 600, color: fg, marginBottom: 14 }}>Included in your plan</div>
              <div className="grid sm:grid-cols-2 gap-2">
                {["Unlimited notes", "All database views", "Real-time collaboration", "Web publishing", "API access", "File uploads", "Mobile apps", "Community support"].map(f => (
                  <div key={f} className="flex items-center gap-2">
                    <div style={{ width: 16, height: 16, borderRadius: "50%", background: "rgba(99,87,232,0.14)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <Check size={9} style={{ color: "#6357E8" }} />
                    </div>
          ...
          {active === "language" && (
            <FadeUp>
              ...
              <button style={{ padding: "11px 24px", background: "#6357E8", border: "none", borderRadius: 10, color: "white", fontSize: 14, fontWeight: 500, cursor: "pointer" }}>Save preferences</button>
            </FadeUp>
          )}
        </motion.div>
      </div>
